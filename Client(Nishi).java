...
public class Client extends JFrame implements MouseListener {
	private JPanel totalPanel = (JPanel)getContentPane(); //対戦画面用のJPanel
	private JPanel boardPanel = new JPanel(); //オセロ盤用のJPanel
	...
	private ImageIcon blackIcon, whiteIcon, boardIcon,canPutIcon; //アイコン
	private PrintWriter out;//データ送信用オブジェクト
	private Receiver receiver; //データ受信用オブジェクト
	private Othello game; //Othelloオブジェクト

	// コンストラクタ
	public Client(Othello game) { //Othelloオブジェクトを引数とする
		...
		boardPanel.setSize(row * 45 + 10, row * 45 + 10);
		...
		for(int i = 0 ; i < row * row ; i++){
			...
			if(grids[i].equals("board")){ buttonArray[i] = new JButton(boardIcon);}//盤面状態に応じてアイコンを設定
			if(grids[i].equals("canPut")){ buttonArray[i] = new JButton(canPutIcon);}//盤面状態に応じたアイコンを設定
		...
		totalPanel.add(boardPanel);
		//終了ボタン
		stop = new JButton("終了");//終了ボタンを作成
		totalPanel.add(stop); //終了ボタンをペインに貼り付け
		...
		totalPanel.add(pass); //パスボタンをペインに貼り付け
		...
		//色表示用ラベル
		colorLabel = new JLabel("黒: " + game.getBlack() + "  白: " + game.getWhite());//色情報を表示するためのラベルを作成
		colorLabel.setBounds(10, row * 45 + 60 , row * 45 + 10, 30);//境界を設定
		totalPanel.add(colorLabel);//色表示用ラベルをペインに貼り付け
		//手番表示用ラベル
		turnLabel = new JLabel("今の手番は" + game.getTurn() + "です");//手番情報を表示するためのラベルを作成
		turnLabel.setBounds(10, row * 45 + 120, row * 45 + 10, 30);//境界を設定
		totalPanel.add(turnLabel);//手番情報ラベルをペインに貼り付け
	}
	...
	public void updateDisp(){
		...
		if(!game.isGameover()) {
			colorLabel.setText("黒: " + game.getBlack() + "  白: " + game.getWhite());
			turnLabel.setText("今の手番は" + game.getTurn() + "です");
		}else {
			colorLabel.setText("黒: " + game.getBlack() + "  白: " + game.getWhite() + " winner: " + game.checkWinner());
			turnLabel.setText("試合終了です");
		}
	}
		...
		switch(IsInteger(command)) {
			case 1://マスが押されたら
				...
				if(game.putStone(Integer.parseInt(command), game.getTurn(), true)) {
					game.changeTurn();//trueなら置いてchangeTurn
					updateDisp();//更新
				}
				break;
			case 0://マス以外のボタンなら
				switch(command) {
					case "pass"://passならchangeTurn
						game.changeTurn();
						updateDisp();//更新
						break;
					case "stop"://stop時点でcheckWinner
						colorLabel.setText("黒: " + game.getBlack() + "  白: " + game.getWhite() + " winner: " + game.checkWinner());
						turnLabel.setText("試合終了です");
						break;
				}
	...
	public static void main(String args[]){
		...
		Client oclient = new Client(game); //引数としてオセロオブジェクトを渡す
